...
//-----------------------------------------------------------------------------
// Helper macros for cross-platform safe string copying and localtime
//-----------------------------------------------------------------------------
#ifdef _MSC_VER
    // Windows: use the secure functions.
    #define MY_LOCALTIME(result, timep) localtime_s(result, timep)
    #define MY_STRCPY(dest, size, src) strcpy_s(dest, size, src)
    // For two-argument calls (when the destination is a fixed-size array)
    #define MY_STRCPY2(dest, src) strcpy_s(dest, src)
#else
    // UNIX: use snprintf and localtime_r.
    #define MY_LOCALTIME(result, timep) localtime_r(timep, result)
    #define MY_STRCPY(dest, size, src) snprintf(dest, size, "%s", src)
    // Here we assume that 'dest' is a fixed-size array so sizeof(dest) works.
    #define MY_STRCPY2(dest, src) snprintf(dest, sizeof(dest), "%s", src)
#endif
...
namespace FileDialog {
    ...
    void ShowFileDialog(bool* open, char* buffer, unsigned int buffer_size, FileDialogType type = FileDialogType::OpenFile) {
        ...
        if (open) {
            // If something already exists in the buffer, try to use that as the initial path.
            if (!initial_path_set && strlen(buffer) > 0) {
                ...
                if (std::filesystem::is_directory(path)) {
                    ...
                    if (std::filesystem::exists(path)) {
                        ...
                        // Invalid path; use the current path.
            ...
            // Sort files based on the selected column.
            ...
            // Use the native path separator.
            char sep = std::filesystem::path::preferred_separator;
            std::string selected_file_path = file_dialog_current_path +
                (file_dialog_current_path.back() == sep ? "" : std::string(1, sep)) +
                (file_dialog_current_folder.size() > 0 ? file_dialog_current_folder : file_dialog_current_file);
            // Note: Using sizeof(buf) on a pointer does not work as on arrays.
            ...
            if (ImGui::BeginPopup("NewFolderPopup", ImGuiWindowFlags_Modal)) {
                ...
                if (ImGui::Button("Create##1")) {
                    if (strlen(new_folder_name) <= 0) {
                        MY_STRCPY2(new_folder_error, "Folder name can't be empty");
                    }
                    else {
                ...
                if (ImGui::Button("Cancel##1")) {
                    MY_STRCPY2(new_folder_name, "");
                    MY_STRCPY2(new_folder_error, "");
                    ImGui::CloseCurrentPopup();
                }
                // Use a format string literal to avoid warnings.
            ...
            if (ImGui::Button("Choose")) {
                if (type == FileDialogType::SelectFolder) {
                    if (file_dialog_current_folder == "") {
                        MY_STRCPY2(file_dialog_error, "Error: You must select a folder!");
                    }
                    else {
                        auto path = file_dialog_current_path +
                            (file_dialog_current_path.back() == sep ? "" : std::string(1, sep)) +
                            file_dialog_current_file;
                        MY_STRCPY(buffer, path.length() + 1, path.c_str());
                        MY_STRCPY2(file_dialog_error, "");
                        reset_everything();
                    }
                }
                    ...
                    if (file_dialog_current_file == "") {
                        MY_STRCPY2(file_dialog_error, "Error: You must select a file!");
                    }
                    else {
                        auto path = file_dialog_current_path +
                            (file_dialog_current_path.back() == sep ? "" : std::string(1, sep)) +
    ...
    // A convenience wrapper assuming a 500‑character buffer.
