        ...
        if (currNodeIndex < 0) {
            if (currNodeIndex != JTX_INT_MIN) {
                ...
                for (size_t i = firstTriangleOffset; i < firstTriangleOffset + numTriangles; i += 4) {
                    ...
                    __m128 t0x = _mm_sub_ps(ox, v0x);
                    __m128 t0y = _mm_sub_ps(oy, v0y);
                    __m128 t0z = _mm_sub_ps(oz, v0z);

                    // Barycentric U
                    __m128 b1     = JTX_SIMD_DOT(t0x, t0y, t0z, crossX, crossY, crossZ);
                    __m128 b1Mask = _mm_and_ps(_mm_cmpge_ps(b1, _mm_setzero_ps()), _mm_cmple_ps(b1, _mm_set1_ps(1.0f)));
#endif
                    __m128 qx = _mm_sub_ps(_mm_mul_ps(t0y, v0v1z), _mm_mul_ps(t0z, v0v1y));
                    __m128 qy = _mm_sub_ps(_mm_mul_ps(t0z, v0v1x), _mm_mul_ps(t0x, v0v1z));
                    __m128 qz = _mm_sub_ps(_mm_mul_ps(t0x, v0v1y), _mm_mul_ps(t0z, v0v1x));
        ...
        if (currNodeIndex < 0) {
            if (currNodeIndex != JTX_INT_MIN) {
                ...
                for (size_t i = firstTriangleOffset; i < firstTriangleOffset + numTriangles; i += 4) {
                    ...
                    __m128 t4 = _mm_blendv_ps(t4, root, mask);
                    __m128 u4 = _mm_blendv_ps(u4, b1, mask);
                    __m128 v4 = _mm_blendv_ps(v4, b2, mask);
