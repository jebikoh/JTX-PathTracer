...
vec3 IntegrateNEE(ray r, const Scene &scene, const BVH &bvh, int maxDepth, Sampler &rng) {
    ...
    while (true) {
        ...
        if (!bSpecularBounce) {
            ...
            const float dist  = SafeSqrt(dist2);
            wi /= dist;
    ...
    while (true) {
        ...
        if (parameters.emission) {
            if (depth == 0 || bSpecularBounce) {
                ...
                // Since we hit a light source, we need to apply MIS to figure out how to weight it
                //
                ...
                float probLight      = 1.0f / nLights * scene.TrianglePDF(triIsect.index) * dist2 / cosTheta;
        ...
        //
        // If the previous bounce was specular, we skip direct lighting sampling since specular
        // surfaces are modeled with a dirac delta function
        auto Ld = vec3(0.0f);
        if (!bSpecularBounce && nLights > 0) {
            // Randomly select a light
            // TODO: handle case for IBL
            const auto index = static_cast<int32_t>(rng.Sample(nLights));
