...
void Display::initUI() const {
    ...
    const auto scale = getWindowScale();
    io.Fonts->AddFontFromFileTTF("../assets/proggyvector.ttf", 14 * scale);
...
int Display::getWindowScale() const {
    int wp, hp;
    SDL_GetWindowSizeInPixels(window_, &wp, &hp);
