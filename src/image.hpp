...
public:
    ...
    float getTexel(const int u, const int v, const uint32_t c0) const {
        int wrappedU = u % width_;
        if (wrappedU < 0) {
            wrappedU += width_;
        }
    ...
    float getTexel(const float u, const float v, const uint32_t c0) const {
        const int x0 = static_cast<int>(u * width_);
        const int y0 = static_cast<int>(v * height_);
        return getTexel(x0, y0, c0);
    }

    Vec2f getTexel(const int u, const int v, const uint32_t c0, const uint32_t c1) const {
        int wrappedU = u % width_;
        if (wrappedU < 0) {
            wrappedU += width_;
        }
    ...
    Vec2f getTexel(const float u, const float v, const uint32_t c0, const uint32_t c1) const {
        const int x0 = static_cast<int>(u * width_);
        const int y0 = static_cast<int>(v * height_);
    ...
    float getTexel(const Vec2f &uv, const uint32_t c0) const {
        return getTexel(uv.x, uv.y, c0);
    }

    Vec2f getTexel(const Vec2f &uv, const uint32_t c0, const uint32_t c1) const {
